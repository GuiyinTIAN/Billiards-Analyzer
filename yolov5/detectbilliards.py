...
def format_output(locations, classes, confidences):
    """格式化输出检测结果"""
    # 表头
    header = f"{'ID':^8} | {'Location [x1, y1, x2, y2]':^30} | {'Class':^6} | {'Confidence':^10}"
    separator = "-" * len(header)
    
    # 构建输出字符串
    output = [separator, header, separator]
    
    for i, (loc, cls, conf) in enumerate(zip(locations, classes, confidences)):
        # 转换numpy数组为Python原生类型
        loc = loc.astype(int).tolist() if isinstance(loc, np.ndarray) else loc
        cls = int(cls) if isinstance(cls, np.ndarray) else cls
        conf = float(conf) if isinstance(conf, np.ndarray) else conf
        
        # 格式化每行数据
        line = (f"{f'Object {i+1}':^8} | "
                f"[{loc[0]:4d}, {loc[1]:4d}, {loc[2]:4d}, {loc[3]:4d}] | "
                f"{cls:^6} | "
                f"{conf:^10.4f}")
        output.append(line)
...
def calculate_center(box):
    """
    计算矩形框的中心点坐标（适配 [x1, y1, x2, y2] 格式）
    
    参数:
        box: 矩形框坐标，格式为 [x1, y1, x2, y2]（NumPy数组或列表）
...
def main(opt):
    ...
    for result in run(**vars(opt)):
        ...
        location = [x.cpu().numpy() for x in pred[0][:, :4]]
        conf = [x.cpu().numpy() for x in pred[0][:, 4]]
        cls = [x.cpu().numpy() for x in pred[0][:, 5]]
        ...
        # for i in cls:
        #     if target == i:
        #         target_index = cls.index(target)
        #         break
        # if len(location) > 0 and target_index != -1:
        #     x_center, y_center = calculate_center(location[target_index])  # 计算第一个框的中心点坐标
        #     error = x_center - 960
        #     if x_center > 980: command = "right"
        #     elif x_center < 940: command = "left"
        #     else: command = "straight"
        #     yield {"command": command, "error": float(error)}
        # else:
        #     yield {"command": "no_target", "error": None}
...
if __name__ == "__main__":
    opt = parse_opt()
    main(opt)
    for result in main(opt):
        print(format_output(result['location'], result['class'], result['confidence']))
        data = []
        for loc, cls, conf in zip(result['location'], result['class'], result['confidence']):
            # 转换numpy数组到Python原生类型
            loc_tuple = tuple(map(int, loc))  # 将location坐标转为整数tuple
            class_id = int(cls)               # 将class转为整数
            confidence = round(float(conf), 3)         # 将confidence转为Python float
            
            # 合并成 (location, class, confidence) 的tuple
            data.append((loc_tuple, class_id, confidence))
        print(data)
